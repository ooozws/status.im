...
To send a message, use the `chat.command/send-message` or `chat.command/send-plain-text-message`
...
Note that `on-send` is then ignored. Messages views still relies on `preview` and `short-preview`.
